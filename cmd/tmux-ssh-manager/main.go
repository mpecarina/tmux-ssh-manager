...
func runSSHWrapperSubcommand(cfg *manager.Config, args []string, execReplace bool) error {
	...
	useConnect := false
	credLookupHostKey := strings.TrimSpace(effectiveHostName) // keychain host key: use effective HostName if present
	credLookupUser := strings.TrimSpace(user)                 // keychain account: resolved effective user, may still be empty
...
func runCredSubcommand(args []string) error {
	...
	case "set":
		...
		return manager.CredSet(host, username, kind)
	case "get":
