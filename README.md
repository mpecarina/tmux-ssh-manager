...
This project is designed to replace "SSH session managers" (e.g., SecureCRT) for network engineers by combining:
...
The Bubble Tea TUI supports a vim-style command bar, intended to be a "SecureCRT command bar":
...
- Why this is useful:
  - Migration path from SecureCRT: keep using your SSH config; no need to duplicate per-host settings in YAML.
...
A tmux plugin and Go-backed CLI that turns tmux into a comfortable SSH session manager for network engineers coming from SecureCRT, but excited about iTerm2, zsh/oh-my-zsh, Catppuccin themes, and tmux/tpm.
...
The goal is to make session selection and SSH launching feel like SecureCRT’s "Sessions" pane, but entirely inside your tmux workflow on macOS/iTerm2.
...
- Import helpers for SecureCRT session exports
...
## SecureCRT to tmux mapping

- SecureCRT "Sessions" folder → `groups` in `hosts.yaml`
